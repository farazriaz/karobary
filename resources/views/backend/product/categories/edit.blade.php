            ...
            <div class="card-body p-0">
                ...
                <form class="p-4" action="{{ route('categories.update', $category->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group row" id="category">
                        ...
                        <div class="col-md-9">
                            <select class="select2 form-control aiz-selectpicker" name="parent_id" id="category_id" data-toggle="select2" data-placeholder="Choose ..."data-live-search="true" data-selected="{{ $category->parent_id }}">
...
$(document).on('change', '[id^="category_id"]', function() {  
        ...
        var id = $(this).attr('id');
        var categoryNumber = (Number(id.match(/\d+/) || 1))+1;        
        if(categoryNumber > 2){return}
        if(categoryNumber == 2){
            $(`#category2`).empty();   
            $(`#category3`).empty();   
        }else{
            $(`#category${categoryNumber}`).empty();   
        }
        ...
        $.ajax({
            ...
            // /    var obj = JSON.parse(data);
                ...
                options += `<option value="">Select Parent Subcategory ${categoryNumber-1}</option>`
                for(category of data.data){
                    options += `<option value="${category.id}">${category.name}</option>`
                }
                $(`#category${categoryNumber}`).append(`\
                <div class="form-group row" id="category${categoryNumber}">\
                            <label class="col-md-3 col-from-label">Parent Subcategory ${categoryNumber-1}</label>\
                            <div class="col-md-9">\
                                <select class="form-control aiz-selectpicker" name="parent_id${categoryNumber}" id="category_id${categoryNumber}" data-live-search="true">\                                                                                                        
                                ${options}
                                </select>\
                            </div>\
                        </div>\
